...
vec4 shadeLine( const in vec4 color )
{
    ...
    float mixVal;
    if ( d < line.width - 0.10 )
    {
        mixVal = 1.0;
    }
    else if ( d > line.width + 0.1 )
    {
        mixVal = 0.0;
    }
    else
    {
        float x = d - ( line.width - 0.10 );
        mixVal = exp2( -4.0 * ( x * x ) );
    }
    mixVal*=line.color[3];
    //vec4 ret=mix( color, line.color, mixVal );
vec4 ret;
    if(mixVal>0){
        ret=line.color;
    }else{
        ret=mix( color, line.color, mixVal );
    }
    return ret;
}
...
void main()
{
    ...
    //vec4 color =  adsModel( fs_in.position, normalize( fs_in.normal ) );
